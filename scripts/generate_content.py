...
def call_kimi(profile_str, hot_text, date_ctx):
    ...
    if hot_text.strip():
        hot_instr = """以下是今日多平台真实热榜(抖音/微博/头条,近2天热点)。你的任务是基于这些热点创作选题,但必须遵守以下原则:

【核心原则】热点是引子,不是目的。选题本身必须对28-45岁妈妈有实用价值。如果一条热点与教育/亲子/孩子学习生活的关联需要"硬凑"(例如名人去世→识字游戏、明星八卦→育儿道理),直接跳过,不要强行改编。

【选题方向配额(10条必须覆盖)】
- 3条 低年级学习(数学思维/识字/计算/背单词/英语启蒙)
- 2条 陪写作业(高效陪写/不吼不叫/时间管理/专注力)
- 2条 生活习惯(作息/自理/手机管理/收纳整理)
- 2条 情绪疏导(孩子哭闹/厌学/亲子冲突/压力释放)
- 1条 亲子日常(亲子关系/家庭氛围/陪伴妙招)

【如何用热点】
- 从热榜中找与上述方向有自然关联的热点(如教育政策/亲子话题/孩子相关社会事件/生活趋势/情绪共鸣类热点)
- 找到自然关联的热点→用它作为选题的切入角度,sourceHot填该热点标题
- 找不到自然关联的→基于方向+季节自行创作,sourceHot填"季节延伸"
- 宁可多几条"季节延伸",也不要硬凑热点关联。实用性 > 时效性

【反面示例(禁止)】名人去世硬扯到学习、明星八卦硬扯到育儿、国际新闻硬扯到教育——这些关联牵强、用户会觉得莫名其妙。"""
    else:
...
1.【爆款视频】生成10条抖音教育赛道选题,严格按上述配额(3学习+2作业+2生活+2情绪+1亲子)。
  选题要像真实抖音爆款,有吸引力且实用。优先用有自然关联的热点做切入角度;没有合适热点的就基于季节+痛点直接创作,不要硬凑。
  每条字段:
  - title:选题标题(像真实抖音爆款标题,有吸引力,15-25字)
  - keywords:2-3个关键词
  - content:中心内容(2-3句概括这条视频拍什么,让妈妈看了觉得"这个我能用")
  - platform:固定"抖音"
  - viralReason:为什么会火(1-2句)
  - searchKeyword:抖音搜索关键词(3-6字,用于在抖音搜到真实同类视频)
  - sourceHot:来源热点标题(从热榜中选有自然关联的一条原标题;无自然关联填"季节延伸")
2.【爆款二创】10条,把上述10个选题方向改编成该博主能直接拍的具体二创内容。同样遵守配额,同样不要硬凑热点。每条:
  - title:二创标题(你的改编)
